...
for entry in data:
    ...
    # 只处理(4,8)网格形状和指定批次大小
    if mesh_shape != (4, 8) or batch not in batch_sizes:
        continue
    
    # 初始化数据结构
    if model not in throughput_data:
        throughput_data[model] = {}
    hardware_key = (hardware["comp_TFLOPS"], hardware["BW_GBPS"])
    ...
    # compute_balancing 使用 static (通常作为基准)
...
# 固定标签位置 - 修复未定义变量问题
# model_label_y = -global_max_throughput * 0.015
# hardware_label_y = -global_max_throughput * 0.008
# batch_label_y = -global_max_throughput * 0.001
lower_y=0 # 更改为0以适应条形图
model_label_y = lower_y-0.25
hardware_label_y = lower_y-0.15
...
for model_idx, model in enumerate(models):
    ...
    for hw_idx, hardware in enumerate(hardware_configs):
        hardware_key = (hardware["comp_TFLOPS"], hardware["BW_GBPS"])
        ...
        # 添加硬件配置标签 - 使用固定高度
        hw_label = f"{hardware['comp_TFLOPS']}TFLOPS, {hardware['BW_GBPS']}GB/s"
...
# 保存图形
save_dir = "evaluation/figs/adaptive/throughput_comparison.pdf"
plt.savefig(save_dir, bbox_inches='tight')
print(f"Figure saved at {save_dir}")
save_dir = "evaluation/figs/adaptive/throughput_comparison.png"
plt.savefig(save_dir, bbox_inches='tight')
print(f"Figure saved at {save_dir}")